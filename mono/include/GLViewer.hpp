...
#define MOUSE_WHEEL_SENSITIVITY 0.1f
#define MOUSE_T_SENSITIVITY 0.02f
...
class Axis : public Simple3DObject {
public:
    Axis() = default;
    Axis(const sl::Transform &rt);
private:
};
...
public:
    ...
    void exit();
    bool isAvailable();
    void init(int argc, char **argv, sl::MODEL camera_model, sl::COORDINATE_SYSTEM);
    void updateData(const sl::Transform &zed_rt, const sl::Transform &zed_aruco_rt, const std::string &str_t, const std::string &str_r, sl::POSITIONAL_TRACKING_STATE state);
    void setArucoTags(const std::map<int, sl::Transform> &aruco_tags);
    void setShowArucoPath(bool showAruco);
...
private:
    ...
    Simple3DObject zedArucoModel;
    Simple3DObject zedArucoPath;
    std::vector<Axis> aruco_axes_;

    std::vector<sl::float3> vecPath;
    std::vector<sl::float3> vecArucoPath;
    std::mutex mtx;
    bool updateZEDposition;
    bool showAruco = false;
    ...
    sl::COORDINATE_SYSTEM coord_sys_;
    CameraGL camera_;
