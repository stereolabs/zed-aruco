...
void GLViewer::init(int argc, char **argv, sl::MODEL camera_model, sl::COORDINATE_SYSTEM coord_sys) {
    glutInit(&argc, argv);
    ...
    zedArucoPath.setDrawingType(GL_LINE_STRIP);
    ...
    if(model){
        ...
        for (auto it: model->part) {
            auto color = it.color;
            color = color + (1.0f - color) * 0.5f;
            color.x = 0.9f;
            fillZED(it.nb_triangles, model->vertices, it.triangles, color, &zedArucoModel);
        }

        delete model;
    }
    zedModel.pushToGPU();
    zedArucoModel.pushToGPU();
...
void GLViewer::update() {
    ...
    if (updateZEDposition) {
        ...
        zedArucoPath.clear();
        sl::float3 clrAruco(0.9f, 0.2f, 0.1f);
        for (unsigned int i = 1; i < vecArucoPath.size(); i++) {
          float fade = (i*1.f) / vecArucoPath.size();
          sl::float3 new_color = clrAruco * fade;
          zedArucoPath.addPoint(vecArucoPath[i], new_color);
        }
        zedArucoPath.pushToGPU();
        updateZEDposition = false;
    }
    mtx.unlock();
}
...
void GLViewer::draw() {
    ...
    glLineWidth(4.f);
    for (auto &aruco : aruco_axes_) {
      glUniformMatrix4fv(mainShader.MVP_Mat, 1, GL_FALSE, (sl::Transform::transpose(aruco.getModelMatrix()) *  sl::Transform::transpose(vpMatrix)).m);
      aruco.draw();
    }
  ...
  // Move the ZED 3D model to correct position
    ...
    if (showAruco) {
      glUniformMatrix4fv(mainShader.MVP_Mat, 1, GL_FALSE, (sl::Transform::transpose(zedArucoModel.getModelMatrix()) *  sl::Transform::transpose(vpMatrix)).m);
      zedArucoModel.draw();
    }
...
void GLViewer::updateData(const sl::Transform &zed_rt, const sl::Transform &zed_aruco_rt, const std::string &str_t, const std::string &str_r, sl::POSITIONAL_TRACKING_STATE state) {
    mtx.lock();

    auto coordinateTransform = sl::getCoordinateTransformConversion4f(coord_sys_, sl::COORDINATE_SYSTEM::RIGHT_HANDED_Y_UP);
    auto coordinateTransformInv = sl::getCoordinateTransformConversion4f(sl::COORDINATE_SYSTEM::RIGHT_HANDED_Y_UP, coord_sys_);

    sl::Transform pose = coordinateTransform * zed_rt * coordinateTransformInv;

    vecPath.push_back(pose.getTranslation());
    zedModel.setRT(pose);

    sl::Transform aruco_pose = coordinateTransform * zed_aruco_rt * coordinateTransformInv;

    vecArucoPath.push_back(aruco_pose.getTranslation());
    zedArucoModel.setRT(aruco_pose);
...
void GLViewer::setArucoTags(const std::map<int, sl::Transform> &aruco_tags) {
    auto coordinateTransform = sl::getCoordinateTransformConversion4f(coord_sys_, sl::COORDINATE_SYSTEM::RIGHT_HANDED_Y_UP);

    for (const auto &tf : aruco_tags) {
        sl::Transform pose = coordinateTransform * tf.second;
        aruco_axes_.emplace_back(pose);

        aruco_axes_.back().pushToGPU();
    }
}
void GLViewer::setShowArucoPath(bool showAruco_) {
    showAruco = showAruco_;
}
...
Axis::Axis(const sl::Transform &rt) : Simple3DObject(sl::Translation(10, 1, 0), false) {
  setPosition(sl::Translation(0, 0, 0));

  setDrawingType(GL_LINES);

  float axisLimit = 0.3f;

  sl::float3 x_clr(255,0,0);
  x_clr /= 255.f;
  sl::float3 y_clr(0,255,0);
  y_clr /= 255.f;
  sl::float3 z_clr(0,0,255);
  z_clr /= 255.f;
  ...
  addLine( origin, xPt,  x_clr);
  addLine( origin, yPt,  y_clr);
  addLine( origin, zPt,  z_clr);
